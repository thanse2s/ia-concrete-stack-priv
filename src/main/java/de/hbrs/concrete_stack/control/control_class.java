...
public class control_class implements ManagePersonal {
    ...
    /*
     * Generates a List containing all EvaluationRecords and filters afterwards.
     */
    public List<EvaluationRecord> readAllEvaluationRecords() {
        this.connectAndOpen();
        List<EvaluationRecord> result = new ArrayList<>();
        MongoCursor<Integer> yearIt = performanceRecords.distinct("year", Integer.class).iterator();
        while (yearIt.hasNext()) {
            int year = yearIt.next();
            for (Integer integer : salesman.distinct("employee_id", Integer.class)) {
                result.add(readEvaluationRecords(integer, year));
            }
        }
        this.disconnectAndClose();
        return result;
