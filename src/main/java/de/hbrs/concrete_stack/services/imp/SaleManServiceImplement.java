...
public class SaleManServiceImplement implements SalesManService {
    ...
    public List<SalesMan> readAll() {
        salesMans = control.querySalesMan(exists("employee_id"));
        return salesMans;
    }
    ...
    public boolean update(long id, SalesMan salesMan) {
        try{
            ...
            if(!tmp.getLastName().equals(salesMan.getLastName())) {
                control.changeSalesMan((int) id, "lastname", salesMan.getLastName());
            }
            if(!tmp.getDepartment().equals(salesMan.getDepartment())) {
                control.changeSalesMan((int) id, "department", salesMan.getLastName());
            }
